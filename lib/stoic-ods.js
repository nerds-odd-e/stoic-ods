...
function parseOds(filePath, done) {
  parseOdsStyles(filePath, function(e, odsStyles) {
    setImmediate(function() {
      parseOdsContent(filePath, odsStyles, done);
    });
  });
}
...
function parseOdsStyles(filePath, done) {
  ...
  .on('entry', function(entry) {
    ...
    console.log(JSON.stringify(odsStylesHandler, null, 2));
    done(e, odsStylesHandler.getOdsStyles());
  });
}

function parseOdsContent(filePath, odsStyles, done) {
  ...
  .on('entry', function(entry) {
    ...
    if (fileName === 'content.xml') {
      odsContentHandler = new OdsContentHandler(entry, odsStyles);
